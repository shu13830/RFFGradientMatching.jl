#=
# ***ODE Parameter Estimation with GPGM***
---
...
gm = GPGM(obs_time, y_obs, prob, ode_name; k=k, state_noise_std=1e-4, obs_noise_std=noise_std)
...
block_X = HMCBlock(gm, [:X]; n_leapfrog=10, step_size=0.05, metric=:diag)

## Block 2: Jointly Sampling the state variable X and ODE parameters θ using HMC
block_Xθ = HMCBlock(gm, [:X, :θ]; n_leapfrog=10, step_size=0.01, metric=:diag)
...
blocks = [[block_X], [block_Xθ], [block_θ]]
probs = [0.4, 0.4, 0.2]
