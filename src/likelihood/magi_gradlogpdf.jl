...
# Reuses existing gradient functions from gradmatch_gradlogpdf.jl with γ=γ_jitter
# Key difference from GPGM: no :γ gradient term
...
function ∇ulogpdf(gm::MAGI, param_dict::Dict{Symbol,Any}, sample_target::Vector{Symbol})
    ...
    if :X in sample_target
        ...
        if gm.β[1] == 0.0
            ...
            ge_dx = gm.β[1] * ∇tx_ulogpdf_e(gm.odegrad, gm.gp, X, θ, gm.γ_jitter)
        end
    ...
    if :θ in sample_target
        ...
        if gm.β[1] == 0.0
            ...
            ge_dtθ = gm.β[1] * ∇tθ_ulogpdf_e(gm.odegrad, gm.gp, X, θ, gm.γ_jitter)
        end
        @assert length(gθ_dtθ) == length(ge_dtθ)
        push!(grad, gθ_dtθ .+ ge_dtθ)
    end
    # NOTE: no :γ gradient for MAGI (γ is fixed)
    if :σ in sample_target
    ...
    if :ϕ in sample_target
        ...
        if gm.β[1] == 0.0
            ...
            ge_dtϕ = gm.β[1] * ∇tϕ_ulogpdf_e(gm.odegrad, gm.gp, X, θ, gm.γ_jitter, ϕ)
        end
